...
test("dataset is a source-traceable whole-year v3 crop model",()=>{
  assert.equal(v3.version,"3.0.0");assert.equal(data.version,"3.0.0");assert.ok(data.crops.length>=35);assert.ok(data.sources.length>=8);
...
test("forecast heat can pause cool-season sowing",()=>{
  const ev=E.evaluateMethod(crop("lettuce"),"direct",ctx({today:date(2026,7,1),fallAnchor:date(2026,11,1),currentForecast:mildForecast(75,96)}));
  assert.equal(ev.state,"blocked");assert.equal(ev.block,"heat");
});

test("temporary shade converts severe heat from a hard stop to managed risk without changing forecast",()=>{
  const c=ctx({today:date(2026,7,1),fallAnchor:date(2026,11,1),currentForecast:mildForecast(75,96),temporaryShade:true});
  const ev=E.evaluateMethod(crop("lettuce"),"direct",c),plan=E.build([crop("lettuce")],c);
...
test("optional measured soil temperature blocks premature warm direct seeding",()=>{
  const ev=E.evaluateMethod(crop("bean"),"direct",ctx({today:date(2026,5,20),fallAnchor:date(2026,10,15),springAnchor:date(2026,5,10),soilTempF:52,currentForecast:mildForecast(45,72)}));
  assert.equal(ev.state,"blocked");assert.equal(ev.block,"soil");
});
...
test("current freeze forecast blocks tender outdoor planting but not indoor starts",()=>{
  ...
  assert.equal(E.evaluateMethod(crop("tomato"),"transplant",c).state,"blocked");assert.notEqual(E.evaluateMethod(crop("tomato"),"indoor",c).state,"blocked");
});

test("actual forecast at a cool crop injury threshold blocks the crop even under a low tunnel assumption",()=>{
  const c=ctx({today:date(2026,10,1),fallAnchor:date(2026,10,15),protectionMode:"low-tunnel",currentForecast:mildForecast(18,48)}),ev=E.evaluateMethod(crop("spinach"),"direct",c);
  assert.equal(ev.state,"blocked");assert.equal(ev.block,"cold-threshold");assert.match(ev.reason,/does not override/i);
});

test("row cover and low tunnel conservatively extend only cool-season planning windows",()=>{
  const lettuce=crop("lettuce"),bean=crop("bean"),base=ctx({today:date(2026,8,15),fallAnchor:date(2026,10,15)});
  const bare=E.latestStart(lettuce,base,"direct","full"),row=E.latestStart(lettuce,{...base,protectionMode:"row-cover"},"direct","full"),tunnel=E.latestStart(lettuce,{...base,protectionMode:"low-tunnel"},"direct","full");
  assert.equal(E.diffDays(row,bare),7);assert.equal(E.diffDays(tunnel,bare),14);
  assert.equal(E.protectionProfile(bean,"row-cover").extensionDays,0);assert.equal(E.protectionProfile(lettuce,"row-cover").extensionDays,7);
});

test("protected spring dates move cool outdoor methods earlier but do not alter indoor starts",()=>{
  const lettuce=crop("lettuce"),c=ctx({today:date(2026,3,1),protectionMode:"row-cover"});
  const directBare=E.methodSpringDate(lettuce,"direct",c.springAnchor),directProtected=E.protectedSpringDate(lettuce,"direct",c),indoorBare=E.methodSpringDate(lettuce,"indoor",c.springAnchor),indoorProtected=E.protectedSpringDate(lettuce,"indoor",c);
  assert.equal(E.diffDays(directBare,directProtected),7);assert.equal(indoorBare.getTime(),indoorProtected.getTime());
});

test("conditional succession repeats from today and stops when the repeat no longer fits",()=>{
  const radish=crop("radish"),good=ctx({today:date(2026,9,1),fallAnchor:date(2026,10,20),currentForecast:mildForecast(45,72)}),row=E.actionFor(radish,good);
  assert.ok(row.succession);assert.equal(E.diffDays(row.succession.date,good.today),7);
  const edge=ctx({today:date(2026,9,24),fallAnchor:date(2026,10,20),currentForecast:mildForecast(40,68)});assert.equal(E.actionFor(radish,edge).succession,null);
});

test("relay recommendations are evaluated at projected harvest rather than copied blindly",()=>{
  const plan=E.build(data.crops,ctx({today:date(2026,5,20),fallAnchor:date(2026,10,15),currentForecast:mildForecast(55,78)})),radish=plan.rows.find(r=>r.crop.id==="radish");
  assert.ok(radish.harvest?.start);assert.ok(radish.relays.length>0);assert.ok(radish.relays.every(r=>r.date.getTime()===radish.harvest.start.getTime()));
});

test("garlic is handled as a regional fall overwinter window",()=>{
  const garlic=crop("garlic"),upcoming=E.evaluateMethod(garlic,"direct",ctx({today:date(2026,9,1),fallAnchor:date(2026,10,15)})),open=E.evaluateMethod(garlic,"direct",ctx({today:date(2026,10,1),fallAnchor:date(2026,10,15)}));
  assert.equal(upcoming.state,"upcoming");assert.equal(open.state,"go");
});

test("frost-light and long-season climates use heat-gated cool-season logic",()=>{
  assert.equal(E.heatMode(25.76,null),"heat-limited");assert.equal(E.heatMode(29.76,date(2026,12,1)),"long-season");
  const houstonWinter=ctx({today:date(2026,12,15),latitude:29.76,fallAnchor:date(2026,12,1),springAnchor:date(2026,2,15),currentForecast:mildForecast(48,72)}),lettuce=E.evaluateMethod(crop("lettuce"),"direct",houstonWinter);
  assert.ok(["go","caution","limited","last-chance"].includes(lettuce.state));assert.equal(lettuce.latest,null);
});
...
test("page leads with plant-today decisions, protected-growing controls and one canonical",()=>{
  assert.match(page,/<h1>What can I plant now\?<\/h1>/);assert.match(page,/Direct sow now/);assert.match(page,/Last call \/ quick harvest/);assert.match(page,/If you plant now, sow again/);assert.match(page,/Use the bed next/);assert.match(page,/Know your soil temperature/);assert.match(page,/id="protection-mode"/);assert.match(page,/id="temporary-shade"/);assert.match(page,/national-planting-engine\.js\?v=20260903-v31/);assert.match(page,/national-planting-page-v3\.js\?v=20260903-v31/);assert.match(pageScript,/protectionMode/);assert.match(pageScript,/temporaryShade/);assert.match(pageScript,/national-planting-v3\.json/);assert.match(page,/rel="canonical" href="https:\/\/chrisizworski\.com\/national-tools\/planting\/"/);assert.doesNotMatch(page,/\/national-tools\/planting\/[a-z-]+\/["']/);
});
